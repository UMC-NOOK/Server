...
--
-- 각 FK 는 NOT VALID 로 추가하여 기존 행 전체 스캔/락 없이 즉시 적용하고,
-- 후속 마이그레이션(V6)에서 VALIDATE CONSTRAINT 로 약한 락 상태로 검증한다.
...
ALTER TABLE library
    ADD CONSTRAINT fk_library_book
        FOREIGN KEY (book_id) REFERENCES book (book_id) ON DELETE CASCADE NOT VALID;
...
ALTER TABLE records
    ADD CONSTRAINT fk_records_library
        FOREIGN KEY (library_id) REFERENCES library (library_id) ON DELETE CASCADE NOT VALID;
...
ALTER TABLE record_images
    ADD CONSTRAINT fk_record_images_record
        FOREIGN KEY (record_id) REFERENCES records (record_id) ON DELETE CASCADE NOT VALID;
