...
public class CategoryInitializer implements ApplicationRunner {
    ...
    public static List<Category> categories = List.of(
            ...
            Category.of(MallType.ETC, "기타", 0)
    );
    @Override
    public void run(ApplicationArguments args) throws Exception {
        log.info("[CATEGORY INIT] 카테고리 동기화 작업을 시작합니다...");

        // 1. DB에 있는 모든 카테고리를 한 번에 가져옴
        ...
        List<Category> newCategories = categories.stream()
                .filter(c -> !existingKeys.contains(c.getMallType() + "_" + c.getCategoryName()))
                .toList();

        // 3. 없는 것만 저장
        if (!newCategories.isEmpty()) {
            categoryRepository.saveAll(newCategories);
            log.info("[CATEGORY INIT] 총 {}개의 새로운 카테고리가 추가되었습니다.", newCategories.size());
        } else {
            log.info("[CATEGORY INIT] 모든 카테고리가 이미 최신 상태입니다.");
        }
    }
}
